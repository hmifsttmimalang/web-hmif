<template>
    <div class="container-fluid">
        ...
        <ConfirmDeleteModal v-if="showDelete" :anggota="anggotaToDelete" @close="showDelete = false" @confirm="deleteAnggota" />

        <!-- Modal Update Anggota -->
        <UpdateAnggotaModal v-if="showUpdate" :anggota="anggotaToUpdate" @close="showUpdate = false" @save="updateAnggota" />
    </div>
</template>

<script setup>
import { ref, computed } from "vue";
import { router } from '@inertiajs/vue3'; // Pake router Inertia
...
function detailAnggota(anggota) {
    // Pindah page detail anggota (nanti backend implementasikan route inertia)
    router.visit(`/admin/anggota/${anggota.id}`)
    // Atau bisa popup/modal kalau butuh
}
...
function deleteAnggota() {
    ...
    // Kalau backend, panggil router.delete/axios.delete ke endpoint kamu
}
...
function updateAnggota(updated) {
    ...
    // Kalau backend, panggil router.patch/axios.patch ke endpoint kamu
}
function importFile(file) {
    alert("File import: " + file.name)
    showImport.value = false
    // Kalau backend, router.post('/admin/import', {file: file}) pakai FormData
}
</script>
